...
class FlutterProject {
  ...
  /// The TVOS sub project of this project.
  TvosProject _tvos;
...
class TvosProject  implements XcodeBasedProject {
  ...
  Directory get ephemeralDirectory => parent.directory.childDirectory('.tvos');
  Directory get _editableDirectory => parent.directory.childDirectory('tvos');
  ...
  Future<void> _regenerateFromTemplateIfNeeded() async {
    ...
    _deleteIfExistsSync(ephemeralDirectory);
    await _overwriteFromTemplate(
      globals.fs.path.join('module', 'tvos', 'library'),
      ephemeralDirectory,
    );
    // Add ephemeral host app, if a editable host app does not already exist.
    if (!_editableDirectory.existsSync()) {
      await _overwriteFromTemplate(
        globals.fs.path.join('module', 'tvos', 'host_app_ephemeral'),
        ephemeralDirectory,
      );
      if (hasPlugins(parent)) {
        await _overwriteFromTemplate(
          globals.fs.path.join('module', 'tvos', 'host_app_ephemeral_cocoapods'),
          ephemeralDirectory,
        );
      }
      // Use release mode so host project can link on bitcode variant.
